#' Relative concentration index for ecological data
#'
#' @description
#' Estimates the Relative Concentration Index (RCI) using grouped ecological
...
#' aggregated unit.
#'
#' The RCI is estimated using the empirical grouped-data concentration-index
#' formula:
#'
#' \deqn{C = (2 / \mu) \sum_i f_i y_i R_i - 1}
#'
#' where \eqn{y_i} is the health indicator for ecological unit \eqn{i},
#' \eqn{f_i} is the population/exposure share, \eqn{R_i} is the fractional rank
#' in the socioeconomic distribution, and \eqn{\mu} is the weighted mean health
#' indicator.
#'
#' The function also computes the Absolute Concentration Index (ACI),
#' \eqn{ACI = \mu C}, as an additional metric. If the health outcome is bounded
#' and bounded corrections are requested, Wagstaff-normalized and
#' Erreygers-corrected RCI estimates are also returned.
#'
#' Smoothed concentration curves, if requested, are computed for visualization
#' only and are not used to estimate the RCI, ACI, bounded corrections, standard
...
#'   For `"proportion"` and `"percentage"`, defaults to 0.
#' @param health_upper_bound Numeric or NULL. Upper bound for bounded outcomes.
#'   For `"proportion"` and `"percentage"`, defaults to 1 after internal
#'   conversion to proportions.
#' @param bounded_corrections Character. One of `"auto"`, `"none"`,
#'   `"wagstaff"`, `"erreygers"`, or `"both"`. `"auto"` computes both
#'   Wagstaff and Erreygers corrections for bounded outcomes.
...
#'
#' @return An object of class `"rci_ineqeco_result"` and
#'   `"aci_rci_ineqeco_result"`, containing:
#' \describe{
#'   \item{summary_stats}{Scalar summary statistics and the primary RCI result.}
...
#' 
#' # Calculate Relative Concentration Index using bootstrap resampling
#' rci <- rci_ineqeco(
#'   data = df_2020,
...
rci_ineqeco <- function(
  ...
  estimation_results <- .rci_estimate(
    prepared = prepared,
    config = config,
    seed = seed
  )
