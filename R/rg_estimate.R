#' Internal Point and Delta-Interval Estimator for Relative Ecological Gaps
#'
#' @description
#' Core estimation engine for the Relative Gap (RG), invoked post-validation and
#' ecological grouping. It computes the relative ratio between the most
#' disadvantaged and most advantaged groups: \eqn{RG = Y_D / Y_A}.
#'
#' Variance estimation strictly follows the `health_se_var` pathway and operates
#' primarily on the log-ratio scale to ensure appropriate asymptotic coverage:
#' - If user-supplied standard errors are provided, group-level variance is pooled
#'   via weighted independent-error propagation and mapped to the log scale via the Delta method.
...
#'
#' @return A complex list of parameter estimates, logarithmic bounds, and closures for resampling.
#'
#' @importFrom stats weighted.mean qnorm
#' @noRd

.rg_estimate_gap <- function(grouped_df,
                             scenario,
                             use_counts,
  ...
  if (!is.finite(y_d_internal) || !is.finite(y_a_internal) ||
      y_d_internal <= 0 || y_a_internal <= 0) {
    stop(
      "The relative ecological gap requires strictly positive disadvantaged and advantaged group estimates. ",
      "At least one extreme group estimate is non-positive or non-finite.",
      call. = FALSE
    )
  }

  point <- y_d_internal / y_a_internal
  metric_multiplier <- 1

  if (is.finite(se_d_internal) && is.finite(se_a_internal)) {
    se_log <- sqrt((se_d_internal / y_d_internal)^2 + (se_a_internal / y_a_internal)^2)
    log_estimate <- log(point)
    log_ci_low <- log_estimate - z * se_log
    log_ci_high <- log_estimate + z * se_log
    ci_low <- exp(log_ci_low)
    ci_high <- exp(log_ci_high)
    se_out <- point * se_log
  ...
  metric_calculator <- function(gdata) {
    d <- group_estimate_internal(gdata, min(gdata$group, na.rm = TRUE))
    a <- group_estimate_internal(gdata, max(gdata$group, na.rm = TRUE))
    if (!is.finite(d) || !is.finite(a) || d <= 0 || a <= 0) return(NA_real_)
    d / a
  }

  interval_scale <- "log_ratio_scale"
