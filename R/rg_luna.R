...
rg_luna <- function(health_indicator_type = NULL,
  ...
  if (!use_num_den) {
    ...
    if (health_indicator_type == "proportion") {
      ...
      res_epi <- epi.2by2(dat = tab, method = "cohort.count", conf.level = conf_level)
  ...
  interpretation_english <- if (health_indicator_type == "rate") {
    if (value > 1) {
      ...
      paste0(
        "The relative gap is ", round(value*100, 2),
        "%, ", conf_level*100 ,"%CI [", round(ci_low*100, 2), "%; ",
        round(ci_high*100, 2), "%]. ",
        "This indicates that the health indicator in the most disadvantaged group is ",
        round(value*100, 2), "% of the number of events per ", factor_legible,
        " individuals of the base population compared to the most advantaged group. ",
        "This is equivalent to the most advantaged group having ", round(1/value, 2),
        " times the number of events per ", factor_legible,
        " individuals of the base population compared to the most disadvantaged group."
      )
    } else if (value == 1) {
      paste0("The relative gap is 1. There are no relative differences per ", factor_legible, " individuals of the population between the extreme social groups.")
    }
  } else if (health_indicator_type == "proportion") {
    if (value > 1) {
      paste0(
        "The relative gap is ", round(value, 2),
        ", ", conf_level*100 , "%CI [", round(ci_low, 2), "; ", round(ci_high, 2), "]. ",
        "This indicates that the health indicator in the most disadvantaged group is ", round(value, 2),
      ...
      paste0(
        ...
        round(ci_high*100, 2), "%]. ",
        "This indicates that the health indicator in the most disadvantaged group is ",
        round(value*100, 2), "% of the most advantaged group. ",
        "This is equivalent to the most advantaged group having ", round(1/value, 2),
        " times the health indicator of the most disadvantaged group."
      )
    } else if (value == 1) {
      paste0("The relative gap is 1. There are no relative differences between the extreme groups.")
    }
  }
  ...
  interpretation_spanish <- if (health_indicator_type == "rate") {
    if (value > 1) {
      ...
      paste0(
        "La brecha relativa es de ", round(value*100, 2),
        "%, ", conf_level*100 ,"%IC [", round(ci_low*100, 2), "%; ",
        round(ci_high*100, 2), "%]. ",
        "Esto indica que el indicador de salud en el grupo más desfavorecido es ",
        round(value*100, 2), "% del número de eventos por cada ", factor_legible,
        " individuos de la población base en comparación con el grupo más favorecido. ",
        "Esto es equivalente a que el grupo más favorecido tiene ", round(1/value, 2),
      ...
      paste0("La brecha relativa es de 1. No hay diferencias relativas por cada ", factor_legible, " individuos de la población entre los grupos sociales extremos.")
    }
  } else if (health_indicator_type == "proportion") {
    if (value > 1) {
      paste0(
        "La brecha relativa es de ", round(value, 2),
        ", ", conf_level*100 , "%IC [", round(ci_low, 2), "; ", round(ci_high, 2), "]. ",
        "Esto indica que el indicador de salud en el grupo más desfavorecido es ", round(value, 2),
      ...
      paste0(
        ...
        round(ci_high*100, 2), "%]. ",
        "Esto indica que el indicador de salud en el grupo más desfavorecido es ",
        round(value*100, 2), "% del grupo más favorecido. ",
        "Esto es equivalente a que el grupo más favorecido tiene ", round(1/value, 2),
        " veces el indicador de salud del grupo más desfavorecido."
      )
    } else if (value == 1) {
      paste0("La brecha relativa es de 1. No hay diferencias relativas entre los grupos extremos.")
    }
  }
  ...
  interpretation_french <- if (health_indicator_type == "rate") {
    if (value > 1) {
      paste0(
        "L'écart relatif est ", round(value, 2), ", ", conf_level*100 , "%IC [", round(ci_low, 2), "; ", round(ci_high, 2), "]. ",
        "Cela indique que l'indicateur de santé dans le groupe le plus défavorisé est ", round(value, 2),
        " fois le nombre d'événements pour chaque ", factor_legible, " individus de la population de base en comparaison avec le groupe le plus favorisé."
      )
    } else if (value < 1) {
      paste0(
        "L'écart relatif est de ", round(value*100, 2),
        "%, ", conf_level*100 ,"%IC [", round(ci_low*100, 2), "%; ",
        round(ci_high*100, 2), "%]. ",
        "Cela indique que l'indicateur de santé dans le groupe le plus défavorisé est ",
        round(value*100, 2), "% du nombre d'événements pour chaque ", factor_legible,
        " individus de la population de base en comparaison avec le groupe le plus favorisé. ",
        "Cela équivaut à ce que le groupe le plus favorisé ait ", round(1/value, 2),
        " fois le nombre d'événements pour chaque ", factor_legible,
        " individus de la population de base en comparaison avec le groupe le plus défavorisé."
      )
    } else if (value == 1) {
      paste0("L'écart relatif est de 1. Il n'y a pas de différences relatives pour chaque ", factor_legible, " individus de la population entre les groupes sociaux extrêmes.")
    }
  } else if (health_indicator_type == "proportion") {
    if (value > 1) {
      paste0(
        "L'écart relatif est de ", round(value, 2),
        ", ", conf_level*100 , "%IC [", round(ci_low, 2), "; ", round(ci_high, 2), "]. ",
        "Cela indique que l'indicateur de santé dans le groupe le plus défavorisé est ", round(value, 2),
        " fois celui du groupe le plus favorisé."
      )
    } else if (value < 1) {
      paste0(
        "L'écart relatif est de ", round(value*100, 2),
        ...
        round(ci_high*100, 2), "%]. ",
        "Cela indique que l'indicateur de santé dans le groupe le plus défavorisé est ",
        round(value*100, 2), "% du groupe le plus favorisé. ",
        "Cela équivaut à ce que le groupe le plus favorisé ait ", round(1/value, 2),
        " fois l'indicateur de santé du groupe le plus défavorisé."
      )
    } else if (value == 1) {
      paste0("L'écart relatif est de 1. Il n'y a pas de différences relatives entre les groupes extrêmes.")
    }
  }
  ...
  interpretation_french <- paste0(
    ...
    if (ci_low <= 1 && ci_high >= 1) {
      " Cependant, ce rapport n'est pas statistiquement significatif."
    } else {
      " Ce rapport est statistiquement significatif."
    }
  )
  ...
  interpretation_portuguese <- if (health_indicator_type == "rate") {
    if (value > 1) {
      paste0(
        "A brecha relativa é ", round(value, 2), ", ", conf_level*100 , "%IC [", round(ci_low, 2), "; ", round(ci_high, 2), "]. ",
        "Isso indica que o indicador de saúde no grupo mais desfavorecido é ", round(summary_table$value, 2),
        " vezes o número de eventos por cada ", factor_legible, " indivíduos da população base em comparação com o grupo mais favorecido."
      )
    } else if (value < 1) {
      paste0(
        "A brecha relativa é de ", round(value*100, 2),
        "%, ", conf_level*100 ,"%IC [", round(ci_low*100, 2), "%; ",
        round(ci_high*100, 2), "%]. ",
        "Isso indica que o indicador de saúde no grupo mais desfavorecido é ",
        round(value*100, 2), "% do número de eventos por cada ", factor_legible,
        " indivíduos da população base em comparação com o grupo mais favorecido. ",
        "Isto é equivalente a que o grupo mais favorecido tem ", round(1/value, 2),
        " vezes o número de eventos por cada ", factor_legible,
        " indivíduos da população base em comparação com o grupo mais desfavorecido."
      )
    } else if (value == 1) {
      paste0("A brecha relativa é de 1. Não há diferenças relativas por cada ", factor_legible, " indivíduos da população entre os grupos sociais extremos.")
    }
  } else if (health_indicator_type == "proportion") {
    if (value > 1) {
      paste0(
        "A brecha relativa é de ", round(value, 2),
        ", ", conf_level*100 , "%IC [", round(ci_low, 2), "; ", round(ci_high, 2), "]. ",
        "Isso indica que o indicador de saúde no grupo mais desfavorecido é ", round(value, 2),
        " vezes o do grupo mais favorecido."
      )
    } else if (value < 1) {
      paste0(
        "A brecha relativa é de ", round(value*100, 2),
        ...
        round(ci_high*100, 2), "%]. ",
        "Isso indica que o indicador de saúde no grupo mais desfavorecido é ",
        round(value*100, 2), "% do grupo mais favorecido. ",
        "Isto é equivalente a que o grupo mais favorecido tem ", round(1/summary_table$value, 2),
        " vezes o indicador de saúde do grupo mais desfavorecido."
      )
    } else if (value == 1) {
      paste0("A brecha relativa é de 1. Não há diferenças relativas entre os grupos extremos.")
    }
  }
  interpretation_portuguese <- paste0(
    interpretation_portuguese,
    if (ci_low <= 1 && ci_high >= 1) {
      " No entanto, essa razão não é estatisticamente significativa."
    } else {
      " Essa razão é estatisticamente significativa."
    }
  )
  ...
  note_french <- if (health_indicator_type == "rate") {
    "Remarque : Les événements correspondent au numérateur et la population de base au dénominateur de l'indicateur de santé. Exemple : RMM."
  } else if (health_indicator_type == "proportion") {
