#' Slope index of inequality (two input modes)
#'
...
#' )
#'
#' @seealso \code{emmeans::emmeans}, \code{sandwich::vcovHC}, \code{Metrics}
#'
#' @references
...
sii_luna <- function(health_indicator_type = NULL,
  ...
  # ───────────────────────────────────────────────────────────────
  # emmeans at extremes (social_position = 1 vs 0), contrast and SII + CI
  # ───────────────────────────────────────────────────────────────
  ...
  contrast_summary <- tryCatch({
    if (is.null(emmeans_extremes)) return(NULL)
    ctr <- contrast(regrid(emmeans_extremes), method = "pairwise")
    summary(ctr, infer = c(TRUE, TRUE), level = conf_level)
  }, error = function(e) NULL)

  # Extract estimate, SE and CI (be defensive about column names)
  sii_est <- NA_real_
  sii_lwr <- NA_real_
  sii_upr <- NA_real_

  if (!is.null(contrast_summary) && nrow(as.data.frame(contrast_summary)) >= 1L) {
    cs <- as.data.frame(contrast_summary)[1, , drop = FALSE]
    if ("estimate" %in% names(cs)) sii_est <- as.numeric(cs$estimate)
    if ("lower.CL" %in% names(cs)) sii_lwr <- as.numeric(cs$lower.CL)
    if ("upper.CL" %in% names(cs)) sii_upr <- as.numeric(cs$upper.CL)

    # Fallback to asymptotic names if present
    if (is.na(sii_lwr) && "asymp.LCL" %in% names(cs)) sii_lwr <- as.numeric(cs$asymp.LCL)
    if (is.na(sii_upr) && "asymp.UCL" %in% names(cs)) sii_upr <- as.numeric(cs$asymp.UCL)
  }
  ...
  if (health_indicator_type == "rate") {
    if (sii_est < 0) {
      part_main_english <- paste0(
        "The Slope Index of Inequality is ", round(sii_est, 2),
        ", ", conf_level*100, "%CI [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "This means that the health indicator is higher at the most disadvantaged extreme. ",
        "In absolute terms, the most disadvantaged extreme has ", abs(round(sii_est, 2)),
        " more events per ", factor_legible, " individuals of the base population compared to the most advantaged extreme."
      )
    } else if (sii_est > 0) {
      part_main_english <- paste0(
        "The Slope Index of Inequality is ", round(sii_est, 2),
        ", ", conf_level*100, "%CI [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "This means that the health indicator is higher at the most advantaged extreme. ",
        "In absolute terms, the most advantaged extreme has ", abs(round(sii_est, 2)),
        " more events per ", factor_legible, " individuals of the base population compared to the most disadvantaged extreme."
      )
    } else if (sii_est == 0) {
      part_main_english <- paste0(
        "The Slope Index of Inequality is ", round(sii_est, 2),
        ", ", conf_level*100, "%CI [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "This indicates that, in absolute terms, there is no difference in the number of events per ", factor_legible, " individuals of the base population between the most advantaged and the most disadvantaged extreme."
      )
    }
    ...
    if (sii_est < 0) {
      part_main_english <- paste0(
        "The Slope Index of Inequality is ", round((sii_est)*100, 2),
        " percentage points (pp), ", conf_level*100, "%CI [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "This means that the health indicator is higher at the most disadvantaged extreme. ",
        "In absolute terms, the difference in the health indicator between the most disadvantaged extreme and the most advantaged extreme is ", abs(round((sii_est)*100, 2)), " percentage points."
      )
    } else if (sii_est > 0) {
      part_main_english <- paste0(
        "The Slope Index of Inequality is ", round((sii_est)*100, 2),
        " percentage points (pp), ", conf_level*100, "%CI [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "This means that the health indicator is higher at the most advantaged extreme. ",
        "In absolute terms, the difference in the health indicator between the most advantaged extreme and the most disadvantaged extreme is ", abs(round((sii_est)*100, 2)), " percentage points."
      )
    } else if (sii_est == 0) {
      part_main_english <- paste0(
        "The Slope Index of Inequality is ", round(sii_est, 2),
        " percentage points (pp), ", conf_level*100, "%CI [", round(sii_lwr*100, 2), "pp; ", round(sii_upr*100, 2), "pp]. ",
        "This indicates that, in absolute terms, there is no difference in the health indicator between the most advantaged and the most disadvantaged extreme."
      )
    }
  }

  if (sii_lwr <= 0 & sii_upr >= 0) {
    part_sig_english <- " However, this difference is not statistically significant."
  } else {
    part_sig_english <- " Additionally, this difference is statistically significant."
  }
  ...
  if (health_indicator_type == "rate") {
    if (sii_est < 0) {
      part_main_spanish <- paste0(
        "El Índice de Desigualdad de la Pendiente es de ", round(sii_est, 2),
        ", ", conf_level*100, "%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Esto significa que el Indicador de Salud es mayor en el extremo más desfavorecido. ",
        "En términos absolutos, el extremo más desfavorecido tiene ", abs(round(sii_est, 2)),
        " más eventos por cada ", factor_legible, " personas de la población base en comparación con el extremo más favorecido."
      )
    } else if (sii_est > 0) {
      part_main_spanish <- paste0(
        "El Índice de Desigualdad de la Pendiente es de ", round(sii_est, 2),
        ...
        "En términos absolutos, el extremo más favorecido tiene ", abs(round(sii_est, 2)),
        " más eventos por cada ", factor_legible, " personas de la población base en comparación con el extremo más desfavorecido."
      )
    } else if (sii_est == 0) {
      part_main_spanish <- paste0(
        "El Índice de Desigualdad de la Pendiente es de ", round(sii_est, 2),
        ", ", conf_level*100, "%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Esto indica que, en términos absolutos, no hay diferencia en el número de eventos por cada ", factor_legible, " personas de la población base entre el extremo más favorecido y el extremo menos desfavorecido."
      )
    }
  } else if (health_indicator_type == "proportion") {
    if (sii_est < 0) {
      part_main_spanish <- paste0(
        "El Índice de Desigualdad de la Pendiente es de ", round((sii_est)*100, 2),
        " puntos porcentuales (pp), ", conf_level*100, "%IC [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "Esto significa que el Indicador de Salud es mayor en el extremo más desfavorecido. ",
        "En términos absolutos, la diferencia del Indicador de Salud en el extremo más desfavorecido y el extremo más favorecido es de ", abs(round((sii_est)*100, 2)), " puntos porcentuales."
      )
    } else if (sii_est > 0) {
      part_main_spanish <- paste0(
        "El Índice de Desigualdad de la Pendiente es de ", round((sii_est)*100, 2),
        " puntos porcentuales (pp), ", conf_level*100, "%IC [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "Esto significa que el Indicador de Salud es mayor en el extremo más favorecido. ",
        "En términos absolutos, la diferencia del Indicador de Salud en el extremo más favorecido y el extremo más desfavorecido es de ", abs(round((sii_est)*100, 2)), " puntos porcentuales."
      )
    } else if (sii_est == 0) {
      part_main_spanish <- paste0(
        "El Índice de Desigualdad de la Pendiente es de ", round(sii_est, 2),
        " puntos porcentuales (pp), ", conf_level*100,"%IC [", round(sii_lwr*100, 2), "pp; ", round(sii_upr*100, 2), "pp]. ",
        "Esto indica que, en términos absolutos, no existe diferencia del Indicador de Salud entre el extremo más favorecido y el extremo más desfavorecido."
      )
    }
  }

  if (sii_lwr <= 0 & sii_upr >= 0) {
    part_sig_spanish <- " Sin embargo, esta diferencia no es estadísticamente significativa."
  } else {
    part_sig_spanish <- " Además, esta diferencia es estadísticamente significativa."
  }
  ...
  if (health_indicator_type == "rate") {
    if (sii_est < 0) {
      part_main_french <- paste0(
        "L'indice d'inégalité de la pente est de ", round(sii_est, 2),
        ", ", conf_level*100, "%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Cela signifie que l'indicateur de santé est plus élevé à l'extrémité la plus défavorisée. ",
        "En termes absolus, l'extrémité la plus défavorisée a ", abs(round(sii_est, 2)),
        " événements de plus pour chaque ", factor_legible, " personnes de la population de base par rapport à l'extrémité la plus favorisée."
      )
    } else if (sii_est > 0) {
      part_main_french <- paste0(
        "L'indice d'inégalité de la pente est de ", round(sii_est, 2),
        ", ", conf_level*100,"%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Cela signifie que l'indicateur de santé est plus élevé à l'extrémité la plus favorisée. ",
        "En termes absolus, l'extrémité la plus favorisée a ", abs(round(sii_est, 2)),
        " événements de plus pour chaque ", factor_legible, " personnes de la population de base par rapport à l'extrémité la plus défavorisée."
      )
    } else if (sii_est == 0) {
      part_main_french <- paste0(
        "L'indice d'inégalité de la pente est de ", round(sii_est, 2),
        ", ", conf_level*100, "%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Cela indique que, en termes absolus, il n'y a pas de différence dans le nombre d'événements pour chaque ", factor_legible, " personnes de la population de base entre l'extrémité la plus favorisée et l'extrémité la moins défavorisée."
      )
    }
  } else if (health_indicator_type == "proportion") {
    if (sii_est < 0) {
      part_main_french <- paste0(
        "L'indice d'inégalité de la pente est de ", round((sii_est)*100, 2),
        " points de pourcentage (pp), ", conf_level*100, "%IC [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "Cela signifie que l'indicateur de santé est plus élevé à l'extrémité la plus défavorisée. ",
        "En termes absolus, la différence de l'indicateur de santé entre l'extrémité la plus défavorisée et l'extrémité la plus favorisée est de ", abs(round((sii_est)*100, 2)), " points de pourcentage."
      )
    } else if (sii_est > 0) {
      part_main_french <- paste0(
        "L'indice d'inégalité de la pente est de ", round((sii_est)*100, 2),
        " points de pourcentage (pp), ", conf_level*100, "%IC [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "Cela signifie que l'indicateur de santé est plus élevé à l'extrémité la plus favorisée. ",
        "En termes absolus, la différence de l'indicateur de santé entre l'extrémité la plus favorisée et l'extrémité la plus défavorisée est de ", abs(round((sii_est)*100, 2)), " points de pourcentage."
      )
    } else if (sii_est == 0) {
      part_main_french <- paste0(
        "L'indice d'inégalité de la pente est de ", round(sii_est, 2),
        " points de pourcentage (pp), ", conf_level*100,"%IC [", round(sii_lwr*100, 2), "pp; ", round(sii_upr*100, 2), "pp]. ",
        "Cela indique que, en termes absolus, il n'existe pas de différence de l'indicateur de santé entre l'extrémité la plus favorisée et l'extrémité la plus défavorisée."
      )
    }
  }

  if (sii_lwr <= 0 & sii_upr >= 0) {
    part_sig_french <- " Cependant, cette différence n'est pas statistiquement significative."
  } else {
    part_sig_french <- " De plus, cette différence est statistiquement significative."
  }
  ...
  if (health_indicator_type == "rate") {
    if (sii_est < 0) {
      part_main_portuguese <- paste0(
        "O Índice de Desigualdade da Inclinação é de ", round(sii_est, 2),
        ", ", conf_level*100, "%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Isto significa que o Indicador de Saúde é maior no extremo mais desfavorecido. ",
        "Em termos absolutos, o extremo mais desfavorecido tem ", abs(round(sii_est, 2)),
        " mais eventos por cada ", factor_legible, " pessoas da população base em comparação com o extremo mais favorecido."
      )
    } else if (sii_est > 0) {
      part_main_portuguese <- paste0(
        "O Índice de Desigualdade da Inclinação é de ", round(sii_est, 2),
        ", ", conf_level*100,"%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Isto significa que o Indicador de Saúde é maior no extremo mais favorecido. ",
        "Em termos absolutos, o extremo mais favorecido tem ", abs(round(sii_est, 2)),
        " mais eventos por cada ", factor_legible, " pessoas da população base em comparação com o extremo mais desfavorecido."
      )
    } else if (sii_est == 0) {
      part_main_portuguese <- paste0(
        "O Índice de Desigualdade da Inclinação é de ", round(sii_est, 2),
        ", ", conf_level*100, "%IC [", round(sii_lwr, 2), "; ", round(sii_upr, 2), "]. ",
        "Isto indica que, em termos absolutos, não há diferença no número de eventos por cada ", factor_legible, " pessoas da população base entre o extremo mais favorecido e o extremo menos desfavorecido."
      )
    }
  } else if (health_indicator_type == "proportion") {
    if (sii_est < 0) {
      part_main_portuguese <- paste0(
        "O Índice de Desigualdade da Inclinação é de ", round((sii_est)*100, 2),
        " pontos percentuais (pp), ", conf_level*100, "%IC [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "Isto significa que o Indicador de Saúde é maior no extremo mais desfavorecido. ",
        "Em termos absolutos, a diferença do Indicador de Saúde no extremo mais desfavorecido e no extremo mais favorecido é de ", abs(round((sii_est)*100, 2)), " pontos percentuais."
      )
    } else if (sii_est > 0) {
      part_main_portuguese <- paste0(
        "O Índice de Desigualdade da Inclinação é de ", round((sii_est)*100, 2),
        " pontos percentuais (pp), ", conf_level*100, "%IC [", round((sii_lwr)*100, 2), "pp; ", round((sii_upr)*100, 2), "pp]. ",
        "Isto significa que o Indicador de Saúde é maior no extremo mais favorecido. ",
        "Em termos absolutos, a diferença do Indicador de Saúde no extremo mais favorecido e no extremo mais desfavorecido é de ", abs(round((sii_est)*100, 2)), " pontos percentuais."
      )
    } else if (sii_est == 0) {
      part_main_portuguese <- paste0(
        "O Índice de Desigualdade da Inclinação é de ", round(sii_est, 2),
        " pontos percentuais (pp), ", conf_level*100,"%IC [", round(sii_lwr*100, 2), "pp; ", round(sii_upr*100, 2), "pp]. ",
        "Isto indica que, em termos absolutos, não existe diferença do Indicador de Saúde entre o extremo mais favorecido e o extremo mais desfavorecido."
      )
    }
  }

  if (sii_lwr <= 0 & sii_upr >= 0) {
    part_sig_portuguese <- " No entanto, essa diferença não é estatisticamente significativa."
  } else {
    part_sig_portuguese <- " Além disso, essa diferença é estatisticamente significativa."
  }
  ...
  note_english <- if (health_indicator_type == "rate") {
    ...
    paste0("Note: Indicator in proportions (0-1). ", "The disadvantaged extreme = posicion_social 0; advantaged = 1.")
  }
  alert_english <- NULL
  if (any(df$health_indicator == 0) && summary_table$selected_model == "linear ln(y)") {
    alert_model <- paste0("Health indicator contains at least one 0. To apply the 'linear ln(y)' model zeros were replaced by ", eps_legible, ". Evaluate whether this compromises the validity of the fit.")
    message(alert_model)
  }
  if (!is.null(alert_english)) note_english <- paste(note_english, alert_english, sep = "\n\n")
  ...
  alert_spanish <- NULL
  if (any(df$health_indicator == 0) && summary_table$selected_model == "linear ln(y)") {
    alerta_modelo <- paste0("Health indicator contiene al menos un 0. Para aplicar el modelo 'linear ln(y)' los ceros fueron reemplazados por ",  eps_legible, ". Evalúe si esto compromete la validez del ajuste.")
    message(alerta_modelo)
  }
  if (!is.null(alert_spanish)) note_spanish <- paste(note_spanish, alert_spanish, sep = "\n\n")


  note_french <- if (health_indicator_type == "rate") {
    paste0("Remarque : Les événements correspondent au numérateur et la population de base au dénominateur. ", "L'extrémité la plus défavorisée = posicion_social 0 ; favorisée = 1.")
  } else {
    paste0("Remarque : Indicateur en proportions (0-1). ", "L'extrémité la plus défavorisée = posicion_social 0 ; favorisée = 1.")
  }
  alert_french <- NULL
  if (any(df$health_indicator == 0) && summary_table$selected_model == "linear ln(y)") {
    alert_french <- paste0("L'indicateur de santé contient au moins un 0. Pour appliquer le modèle 'linear ln(y)' les zéros ont été remplacés par ", eps_legible, ". Évaluez si cela compromet la validité de l'ajustement.")
    message(alert_french)
  }
  if (!is.null(alert_french)) note_french <- paste(note_french, alert_french, sep = "\n\n")
  ...
  if (any(df$health_indicator == 0) && summary_table$selected_model == "linear ln(y)") {
    alerta_modelo <- paste0("O indicador de saúde contém pelo menos um 0. Para aplicar o modelo 'linear ln(y)' os zeros foram substituídos por ", eps_legible, ". Avalie se isto compromete a validade do ajuste.")
    message(alerta_modelo)
  }
  if (!is.null(alert_portuguese)) note_portuguese <- paste(note_portuguese, alert_portuguese, sep = "\n\n")
